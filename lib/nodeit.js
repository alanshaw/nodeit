...
function Nodeit (opts) {
  ...
  var pluginInitTasks = Object.keys(plugins).map(function (id) {
    return function (cb) {
      this.plugin(id, plugins[id], cb)
    }.bind(this)
  }.bind(this))
  
  // When the plugins are ready, nodeit is ready
  async.parallel(pluginInitTasks, function (er) {
    if (er) return console.error("Failed to init built in plugins", er)
    this.emit("ready")
    this.bridge.ready()
  }.bind(this))
...
Nodeit.prototype.open = function (path, cb) {
  ...
  this.bridge.open(path, function (er, path, contents) {
    ...
    var mode = cmUtil.pathToMode(path)
    
    cmUtil.loadMode(mode, function (er) {
      if (er) return cb(er)
...
/**
 * Register a plugin with nodeit. Nodeit will call a function called "create" or "init" on the passed plugin object
 ...
 * @param {Object} plugin The plugin instance (with init method) OR module (with create method)
 * @param {Function} cb Success/failure callback
 */
Nodeit.prototype.plugin = function (id, plugin, cb) {
  console.log("Registering plugin", id)
  
  // Ensure plugin initialisation is async even if plugin author doesn't make it so
  setImmediate(function () {

    if (typeof plugin.create == "function") {
      return plugin.create(this, function (er, instance) {
        if (er) {
          console.error("Failed to create plugin", id)
          return cb(er)
        }

        this.plugins[id] = instance || plugin
        cb(null, this.plugins[id])

      }.bind(this))
    }
    
    if (typeof plugin.init == "function") {
      return plugin.init(this, function (er, instance) {
        if (er) {
          console.error("Failed to initialise plugin", id)
          return cb(er)
        }
    ...
    console.error("Failed to init or create", id)
    cb(new Error(id + " has no init or create function to call"))
 ...
 * @param {String} id
 */
Nodeit.prototype.getPlugin = function (id) {
